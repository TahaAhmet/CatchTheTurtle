...
turtle_colors = ["yellow", "green", "purple", "blue", "red", "orange"]

def setup_target_color():
    for i in range(20):
        target_turtle2=random.choice(turtle_colors)
    target_turtle.hideturtle()
    target_turtle.color(target_turtle2)
    target_turtle.penup()
    ...
    target_turtle.setposition(0, y-45)
    target_turtle.write(arg='Color of target turtle: {}'.format(target_turtle2), move=False, align="center", font=FONT)
...
def make_turtle(x, y):
    ...
    def handle_click(x, y):
        global score
        clicked_turtle = t
        if clicked_turtle.color()[0] == target_turtle.color()[0]:  # Compare colors
            score += 1
            score_turtle.clear()
...
def countdown(time):
    ...
    count_down_turtle.setposition(0, y - 25)
